# FEATURE-13 zora-claude-cui-poc 模块

> 状态：设计中
> 关联模块：`zora-poc/zora-cui-poc/zora-claude-cui-poc`

## 1. 背景

`zora-poc` 用于隔离技术预研，避免实验性依赖进入正式模块。当前需要验证 Java 是否可以构建一个类似 Claude 的 CUI（Command-line User Interface）程序：在终端中提供聊天式交互、命令历史、彩色提示符和模拟流式输出。
...
└── zora-cui-poc/
    ...
    └── zora-claude-cui-poc/
        ...
        └── src/
            ├── main/
            │   ├── java/
            │   │   └── top/ilovemyhome/zora/poc/cui/claude/...
            │   └── resources/
            └── test/
                ├── java/
                └── resources/
...
- `zora-cui-poc`：CUI / terminal UI 技术预研聚合模块，`packaging=pom`。
- `zora-claude-cui-poc`：具体 JLine 聊天式 CUI 实验模块。

虽然 `zora-poc` 的基础原则是实验代码通常放在 `src/test/java`，但本模块需要验证“Java 能否 build 出可运行 CUI 程序”。因此可执行入口和核心交互逻辑放在 `src/main/java`，测试放在 `src/test/java`。该例外仅限本 POC，用于验证真实 CLI 启动体验。
...
| 类 | 职责 |
|---|---|
| `ClaudeCuiApplication` | `main` 入口，创建 JLine `Terminal`、`LineReader` 并启动 shell。 |
| `ClaudeCuiShell` | 主交互循环，读取用户输入、分发命令、输出响应。 |
| `ClaudeCuiCommand` | 命令枚举或解析结果，表示 `HELP`、`CLEAR`、`EXIT`、`CHAT` 等类型。 |
| `ClaudeCuiCommandParser` | 解析 `:help`、`:clear`、`:exit`、`:quit` 和普通聊天输入。 |
...
`zora-claude-cui-poc` 新增 JLine 依赖，作用范围限制在该模块内。建议使用：
...
mvn -pl zora-poc/zora-cui-poc/zora-claude-cui-poc exec:java \
  -Dexec.mainClass=top.ilovemyhome.zora.poc.cui.claude.ClaudeCuiApplication
```
...
| 测试类 | 覆盖内容 |
|---|---|
| `ClaudeCuiCommandParserTest` | 内置命令、空输入、普通聊天输入、未知命令解析。 |
