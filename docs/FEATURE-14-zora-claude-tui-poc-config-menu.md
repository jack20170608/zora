# FEATURE-14 zora-claude-tui-poc 配置菜单

## 背景

`zora-claude-tui-poc` 是一个基于 Java 与 JLine 的 Claude-like 终端交互 POC。此前模块已经具备欢迎页、命令解析、mock assistant 流式输出和退出等基础能力，但缺少可交互修改运行参数的入口。
...
```bash
mvn -pl zora-poc/zora-tui-poc test
```
...
```bash
test -f zora-poc/zora-tui-poc/zora-claude-tui-poc/config/zora-claude-tui.properties
```
